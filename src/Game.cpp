...
void Game::run_game(){
    ...
    game_board_pointer = new Board(6, 7, &player_tokens, 4, 2);

    BotPlayer player1(game_board_pointer, 0);

    player1.set_token('x');

    player_tokens.push_back('x');

    BotPlayer player2(game_board_pointer, 1);

    player2.set_token('o');

    player_tokens.push_back('o');
    ...
    while (!game_won && !game_drawn){
        ...
        while(!valid_move){
            ...
            //std::cout << column_choice << std::endl;
            token_position = this->game_board_pointer->make_move(player_tokens[player_number], column_choice);
            this->game_board_pointer->print_board();
            }
